  ...
  getInfo({ commit, state }) {
    return new Promise((resolve, reject) => {
      getInfo(state.token).then(response => {
        // 🌸获取用户信息:返回数据包含：用户名name、用户头像avatar、routes[返回的标志:
        // 不同的用户应该展示哪些菜单的标记]、roles（用户角色信息）、buttons【按钮的信息：按钮权限用的标记】
        const { data } = response
        console.log(data)
        // vuex存储用户全部的信息
...
const mutations = {
  // 重置state
  RESET_STATE: (state) => {
    Object.assign(state, getDefaultState())// 🍀清除数据
  },
  // 🍀存储token
  ...
  SET_RESULTASYNCROUTES: (state, asyncRoutes) => {
    // ⭐️vuex保存当前用户的异步路由，注意，一个用户需要展示完成路由：常量、异步、任意路由
    state.resultAsyncRoutes = asyncRoutes// 🌸 所有异步路由-需要算
    // 计算出当前用户需要展示所有路由
